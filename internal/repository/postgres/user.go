...
func NewUserRepository(
	...
	return &UserRepositoryImpl{
		BaseRepository: db.NewBaseRepository[domain.User](
			postgres,
		),
	}
}
...
func (r *UserRepositoryImpl) Create(
	...
	return r.BaseRepository.WithTransaction(
		ctx,
		func(txRepo *db.BaseRepository[domain.User]) error {
...
func (r *UserRepositoryImpl) GetByProviderID(
	ctx context.Context,
	providerID string,
) (*domain.User, error) {
	var user domain.User
...
func (r *UserRepositoryImpl) Update(
	...
	return r.BaseRepository.WithTransaction(
		...
		func(txRepo *db.BaseRepository[domain.User]) error {
			var exists bool
			err := txRepo.GetDB().WithContext(ctx).
				Model(&domain.User{}).
				Select("count(*) > 0").
				Where("id = ?", user.ID).
				Find(&exists).Error
