...
func NewProductRepository(
	...
	return &ProductRepositoryImpl{
		BaseRepository: db.NewBaseRepository[domain.Product](
			postgres,
		),
	}
}
...
func (r *ProductRepositoryImpl) Create(
	...
	return r.BaseRepository.WithTransaction(
		ctx,
		func(txRepo *db.BaseRepository[domain.Product]) error {
			if err := r.validateCategoryIsLeaf(
				ctx,
			...
			if err := txRepo.GetDB().
				...
				return fmt.Errorf(
					"%w: %v",
					customErrors.ErrInvalidProductData,
					err,
				)
			}
...
func (r *ProductRepositoryImpl) List(
	ctx context.Context,
) ([]domain.Product, error) {
...
func (r *ProductRepositoryImpl) ListByCategoryID(
	ctx context.Context,
	categoryID string,
) ([]domain.Product, error) {
	var products []domain.Product
...
func (r *ProductRepositoryImpl) validateCategoryIsLeaf(
	ctx context.Context,
	categoryID string,
) error {
	var count int64
	...
	if count > 0 {
		return fmt.Errorf(
			"%w: products can only be attached to leaf categories",
			customErrors.ErrInvalidProductData,
		)
	}
	return nil
