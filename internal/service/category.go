...
func (s *CategoryServiceImpl) GetByID(ctx context.Context, id string) (*domain.Category, error) {
	if id == "" {
		return nil, fmt.Errorf("%w: category ID is required", customErrors.ErrInvalidCategoryData)
	}
...
func (s *CategoryServiceImpl) ListByParentID(ctx context.Context, parentID string) ([]domain.Category, error) {
	if parentID == "" {
		return nil, fmt.Errorf("%w: parent ID is required", customErrors.ErrInvalidCategoryData)
	}
...
func (s *CategoryServiceImpl) Update(ctx context.Context, category *domain.Category) error {
	...
	if category.ParentID != nil {
		if category.ID == *category.ParentID {
			return fmt.Errorf("%w: category cannot be its own parent", customErrors.ErrInvalidCategoryData)
		}
...
func (s *CategoryServiceImpl) Delete(ctx context.Context, id string) error {
	if id == "" {
		return fmt.Errorf("%w: category ID is required", customErrors.ErrInvalidCategoryData)
	}
	...
	if len(subcategories) > 0 {
		return fmt.Errorf("%w: cannot delete category with subcategories", customErrors.ErrInvalidCategoryData)
	}
