...
func (s *CustomerServiceImpl) Create(ctx context.Context, userID string) (*domain.Customer, error) {
	if userID == "" {
		return nil, fmt.Errorf("%w: user ID is required", customErrors.ErrInvalidCustomerData)
	}
	...
	if err == nil && existing != nil {
		return nil, fmt.Errorf("%w: customer already exists for this user", customErrors.ErrInvalidCustomerData)
	}
...
func (s *CustomerServiceImpl) GetByUserID(ctx context.Context, userID string) (*domain.Customer, error) {
	if userID == "" {
		return nil, fmt.Errorf("%w: user ID is required", customErrors.ErrInvalidCustomerData)
	}
